...
* **硬體組成與應用**：
    * **設備名稱**：智慧濾淨呼吸窗
    * **硬體架構**：AMB82-mini 開發板、影像感測模組、語音輸出模組。
    * **核心功能**：透過 Edge AI 進行即時環境/物件辨識，並經由大語言模型（LLM）決策，轉化為語音語意，操作硬體裝置提供即時協助。
* **成果展示**：
    * **硬體組成**：室內與室外雙向空氣品質感測模組（偵測 PM2.5、CO2、VOCs） + 窗戶電動推桿機構 + 窗載式微型進氣風機與 HEPA 濾網 + AI-MCU 執行「微氣候決策代理人」。
    * **功能說明**：在本地端即時交叉比對室內外的空氣品質。當室內 CO2 濃度過高且室外空氣良好時，自動推開窗戶自然通風；若室外空氣污染嚴重（如 PM2.5 超標或有油煙味），則立刻關閉窗戶，並啟動內建的過濾風機，將外氣淨化後引入，確保室內永遠維持鮮氧與潔淨。
...
* **成果展示**：
    ![作業六成果圖一](image-10.png)
    ![作業六成果圖二](image-11.png)
...
* **成果展示**：
    ![作業七成果圖一](image-12.png)
    ![作業七成果圖二](image-13.png)
...
* **硬體架構設計**：
    * **主控核心**：AMB82-mini（利用其強大的 NPU 算力與 H.264 影像處理能力）。
    * **感測周邊**：板載相機模組、VL53L0X 紅外線測距（輔助高度變化偵測）、DHT11（環境溫溼度監控）。
* **AI 協助功能與模型**：
...
這門課讓我明白，未來的嵌入式系統不再只是盲目地將數據上傳雲端，而是要在運算資源有限的 MCU 邊緣端，優化並運行具備智慧的 AI模型，實現低延遲、高隱私且節能的智能裝置。這對我未來在物聯網與 AIoT 領域的專案開發奠定了非常紮實的基礎。